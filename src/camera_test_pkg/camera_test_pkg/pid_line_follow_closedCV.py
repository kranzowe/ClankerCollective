...
TURN_CONFIGS = [
    ...
    {"steps": 50, "steer": -300.0, "speed": DEFAULT_SPEED*0.996},  # turn 2
    {"steps": 30, "steer": -300.0, "speed": DEFAULT_SPEED*0.996},  # turn 3
]
...
class LineFollower(Node):
    ...
    def path_callback(self, msg: Path2D):
        ...
        if all(np.isfinite([pose.theta for pose in msg.poses[-4:]])):
            self.get_logger().info('see valid points in our vertical bands')
