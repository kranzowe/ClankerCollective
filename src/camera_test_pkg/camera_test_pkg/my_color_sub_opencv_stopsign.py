...
class ImageListener(Node):
    def __init__(self):
        ...
        # Publisher to signal stop (True = stopped, False = moving)
        ...
        # Toggle stop sign detection on/off
        self.declare_parameter("stop_sign_detection_enabled", True)
        # Minimum contour area to count as a stop sign blob
        self.declare_parameter("stop_sign_min_area", 1000)
        # How long (seconds) the robot stops when a stop sign is detected
        self.declare_parameter("stop_sign_duration", 2.0)
        ...
        self.stop_sign_duration = 2.0
        self.stop_until = 0.0   # timestamp until which we are stopped
    ...
    def listener_cb(self, data):
        ...
        if self.stop_sign_detection_enabled:
            stop_sign_seen = self.detect_stop_sign(frame)

            if stop_sign_seen and now >= self.stop_until:
                # Start a new stop period
                self.stop_until = now + self.stop_sign_duration
                self.get_logger().info(
                    f"Stop sign detected! Stopping for {self.stop_sign_duration}s."
                )

        # Publish stop state so the controller can act on it
        stop_msg = Bool()
        stop_msg.data = (now < self.stop_until)
        self.stop_pub.publish(stop_msg)

        # While stopped: skip publishing a path so the controller holds position
        if now < self.stop_until:
            cv2.putText(frame, "STOPPED", (width // 2 - 60, 40),
    ...
    def update_params(self):
        ...
        self.stop_sign_duration          = self.get_parameter("stop_sign_duration").value
